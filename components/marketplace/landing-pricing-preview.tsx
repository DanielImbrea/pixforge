  ...
  return (
    <div>
      <div className="text-center mb-10">
        <h2 className="text-2xl font-semibold text-text-primary mb-2">{t('heading')}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        {PLAN_ORDER.map((tier) => {
          ...
          return (
            <Card
              key={tier}
              className={isRecommended ? 'border-2 border-accent relative' : ''}
            >
              {isRecommended && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-accent px-3 py-0.5 text-xs font-medium text-white">
                  {tHome('pricingRecommended')}
                </span>
              )}
              <h3 className="text-lg font-medium text-text-primary mb-1">{t(tier)}</h3>
              ...
              <p className="text-xs text-text-tertiary">{tHome(`planHighlight_${tier}` as 'planHighlight_free')}</p>
            </Card>
          );
        })}
      </div>
